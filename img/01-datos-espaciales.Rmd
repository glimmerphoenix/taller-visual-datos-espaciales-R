...
# (PART) DATOS ESPACIALES {.unnumbered}
...
- Se presentan los principales tipos de datos espaciales.
- Conocerás las diferentes técnicas y modelos disponibles para analizar
datos espaciales.
- Comenzarás a trabajar con datos espaciales usando R y RStudio.
- Descubrirás las principales características de los paquetes de R {sf} y {terra}.
:::
...
```{r, echo=FALSE, out.width="75%", fig.align='center', fig.cap="Figura tomada de [@Lovelace2019], sección 2.3."}
include_graphics("img/explanation_raster.png")
```
...
```{r, echo=FALSE, out.width="75%", fig.align='center', fig.cap="Figura tomada de [@Lovelace2019], sección 2.3."}
include_graphics("img/cont_cat_raster.png")
```
...
Un Sistema de referencia de coordenadas (CRS) define cómo los elementos espaciales de los datos se relacionan con la superficie de la Tierra (u otros cuerpos). Los CRS pueden ser geográficos o proyectados, vamos a ver con detalle cada uno de ellos.

### Sistemas de coordenadas geográficas (GCS)

Los sistemas de coordenadas geográficas identifican cualquier ubicación en la superficie de la Tierra usando dos valores: 

- **longitud**: es la ubicación en la dirección Este-Oeste en la distancia angular desde el plano del primer meridiano.
- **latitud**: es la distancia angular al norte o al sur del plano ecuatorial. 

Dado que la superficie de la Tierra no es una esfera perfecta, los sistemas de coordenadas geográficas pueden trabajar tanto con modelos esféricos como elipsoides. Los modelos esféricos son una aproximación ya que asumen que la Tierra es esférica y puede definirse por un único radio. Como tal aproximación, simplifican los cálculos pero son inexactos. Por el contrario, los modelos elipsoidales (que utilizan dos radios para dibujar el elipsoide: radio ecuatorial y radio polar) son más precisos. 


### Sistemas de referencia de coordenadas proyectadas (PCS)

Los sistemas de referencia de coordenadas proyectadas consisten en transformar la superficie 3D de la Tierra en una superficie 2D, es decir, parejas de valores Este-Norte (X e Y). En este tipo de transformaciones siempre se produce alguna deformación, por lo que, algunas de las propiedades de la superficie de la Tierra (área, dirección, distancia y forma) se distorsionan. Solo es posible mantener intactas una o dos de estas propiedades. Las proyecciones se nombran en función de la propiedad que conservan.

Según las propiedades de la superficie de la Tierra que se mantienen intactas, se distinguen tres tipos de proyecciones: 

- En la **proyección cónica** la superficie de la Tierra se proyecta sobre un cono a lo largo de una sola línea de tangencia o dos líneas de tangencia. Las distorsiones se minimizan a lo largo de estas líneas y aumentan con la distancia a esas líneas. Este tipo de proyección es el más adecuado para mapas de áreas de latitud media.

- En la **proyección cilíndrica** se proyecta la superficie de la Tierra sobre un cilindro. Este tipo de proyección se utiliza para cartografiar el mundo entero.

- En la **proyección plana (azimutal)** cada punto de la superficie de la Tierra se proyecta sobre un plano que es tangente a la Tierra en ese punto. Suele utilizarse para cartografiar regiones polares.

```{r projections, echo=FALSE, purl=FALSE,fig.cap="Geo awesomes <https://geoawesomeness.com/top-7-maps-ultimately-explain-map-projections/map-projections-explained/>"}
knitr::include_graphics("img/Map-Projections-Explained.jpg")
```

`sf_proj_info(type = "proj")` da una lista de las proyecciones disponibles compatibles con la biblioteca PROJ.


### CRS en R

Los paquetes espaciales de R admiten una amplia gama de CRS y utilizan la biblioteca *PROJ*. Existen dos formas recomendadas de describir los CRS en R:

- **SRID** (Spatial Reference System Identifier): es un identificador único para cada sistema de referencia de coordenadas, son de la forma AUTORIDAD: CÓDIGO, por ejemplo, EPSG:4326.
- **WKT218** (well-known text): Describe sistemas de referencia de coordenadas (CRS) y coordina las operaciones entre ellos en forma de cadenas de texto conocidas. Es exhaustivo, detallado y preciso, pero al mismo tiempo, también es más complicado de manejar.

Veamos mediante algunos ejemplos cómo se almacenan los CRS en los objetos espaciales de R. Empezaremos por un conjunto de datos vectoriales que leeremos de un fichero y lo almacenaremos en nuestro objeto `new_vector`. El paquete {sf} proporciona la función `st_crs()` para acceder al CRS de un objeto.

```{r shapes_world, out.width = "75%"}
vector_filepath <- system.file("shapes/world.gpkg",
                               package = "spData")
new_vector <- read_sf(vector_filepath)
st_crs(new_vector)
```

Como podemos ver, el CRS es una lista de dos elementos: 

- `input`: según el archivo de entrada, puede contener una representación SRID o el nombre de un CRS (como en este caso), algunos más antiguos pueden contener una definición de proj4string. 
- `wkt`: almacena la representación WKT2. En nuestro caso particular podemos ver que `new_vector` tiene el elipsoide WGS84, usa el primer meridiano de Greenwich y el orden de los ejes de latitud y longitud. 

Para asignar un sistema de referencia de coordenadas (CRS) a un objeto ya creado, tenemos la función `st_set_crs()`: 
```{r set_crs, out.width = "75%"}
new_vector <- st_set_crs(new_vector, "EPSG:4326")
```

Si estamos trabajando con objetos `SpatRaster`, tenemos la función `crs()` tanto para acceder a la información de CRS del objeto como para modificar el CRS que ya tuviera el objeto: 

```{r raster_crs, out.width = "75%"}
# Consultamos el CRS
crs(raster_file)
plot(raster_file)

# Modificamos el CRS
crs(raster_file) = "EPSG:26912"
plot(raster_file)
```

Tanto la función `st_crs()` como `crs()` **no alteran** los valores o geometrías de las coordenadas. Su finalidad es solo proporcionar información sobre el CRS de un objeto. 
...
- Los datos espaciales y espacio-temporales han adquirido especial relevancia
durante las última décadas.
- El paquete {sf} ofrecen una gran cantidad de funciones que dan soporte a la representación y el análisis de datos espaciales vectoriales.
- El paquete {raster} permite trabajar con datos espaciales en cuadrícula.
- En las siguientes unidades veremos más paquetes que permiten incorporar herramientas
y modelos avanzados de análisis de datos espaciales en R.
:::
